...
class MultiComparisonAnalyzer:
    ...
    def parse_all_results(self, result_files: List[str]) -> Dict[str, Any]:
        """解析所有回测结果文件"""
        all_results = {}
    ...
    def _find_best_combination(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'key_metrics': {
                '年化收益率': f"{best_metrics.get('annualized_return', 'N/A'):.2f}%",
                '夏普比率': f"{best_metrics.get('sharpe_ratio', 'N/A'):.2f}",
                '最大回撤': f"{best_metrics.get('max_drawdown', 'N/A'):.2f}%"
            }
        }
