...
def _passes_today_constraints(df: pd.DataFrame, code: str) -> bool:
    ...
    3. 价格条件（满足其一即可）：
       - 条件A：T日上涨且涨幅 < 4%
       - 条件B：T-1日涨幅 > 9% 且 T日下跌，跌幅在0-3%之间
    4. 量能条件：T日量能 ≥ T-1日量能的70%

    策略意图：选择T日表现温和、未涨停的股票，在T+1日买入
    ⚠️ 注意：T+1日开盘时需人工/程序判断是否高开5%以上，若是则放弃买入
    涨停价按A股规则四舍五入到两位小数: round(T-1日收盘 * (1+limit_ratio), 2)
    """
    if len(df) < 3:  # 需要T-2、T-1、T日数据
        return False

    prev_prev_close = float(df['close'].iloc[-3])  # T-2日收盘价
    prev_close = float(df['close'].iloc[-2])  # T-1日收盘价
    today_close = float(df['close'].iloc[-1])  # T日收盘价（当前价）

    prev_volume = float(df['volume'].iloc[-2])  # T-1日成交量
    today_volume = float(df['volume'].iloc[-1])  # T日成交量
    ...
    if prev_prev_close <= 0:
        return False
    prev_day_pct_change = (prev_close - prev_prev_close) / prev_prev_close

    # 计算T日涨跌幅
    if prev_close <= 0:
        return False
    today_pct_change = (today_close - prev_close) / prev_close

    # 条件A：T日上涨且涨幅 < 4%
    condition_a = (today_pct_change > 0) and (today_pct_change < 0.04)

    # 条件B：T-1日涨幅 > 9% 且 T日下跌，跌幅在0-3%之间
    condition_b = (prev_day_pct_change > 0.09) and (today_pct_change < 0) and (today_pct_change > -0.03)
    ...
    if prev_volume <= 0:
        return False
    if today_volume < prev_volume * 0.7:
        return False
...
def analyze_stock(df: pd.DataFrame, code: str) -> tuple[bool, str]:
    ...
    4. 价格条件（满足其一）：
       - T日上涨且涨幅 < 4%
       - T-1日涨幅 > 9% 且 T日下跌，跌幅在0-3%之间
    5. 量能条件：T日量能 ≥ T-1日量能的70%
    6. 非ST股（在调用侧已过滤）

    ⚠️ T+1日开盘时需额外判断：
    - 排除高开5%以上：若T+1日开盘相对T日收盘高开>=5%，则放弃买入

    选股意图：量能放大 + 位置不高 + T日表现温和 + 量能充足 → T+1日买入机会
    """
