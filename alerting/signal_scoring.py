...
- v1.5: 对称设计+参数化（2025-10-25）
  * 新增：买入信号的极端杀跌降级（对称卖出的极端拉升降级）
  * 参数化：极端行情降级参数可配置（涨跌幅、量比、位置、降级系数）
  * 效果：对称处理极端拉升和极端杀跌，避免拉升中途误卖&杀跌中途误买
"""
...
class SignalScorer:
    ...
    def calc_signal_strength(df, i, signal_type,
        ...
        try:
            ...
            # 🔧 极端行情总分降级（对称设计：拉升和杀跌）
            if len(df_work) >= SignalScorer.MOMENTUM_WINDOW:
                ...
                if signal_type == 'SELL':
                    # 卖出降级：极端强势拉升（涨幅大+巨量+创新高）
                    # 温和拉升、拉升失败等场景保持原样，由其他评分维度处理
                    is_extreme_surge = (
                            price_change_10 > SignalScorer.EXTREME_MOMENTUM_THRESHOLD and
                            vol_ratio > SignalScorer.EXTREME_VOLUME_THRESHOLD and
                            price_position > SignalScorer.EXTREME_POSITION_HIGH
                    )
                    ...
                    is_extreme_fall = (
                            price_change_10 < -SignalScorer.EXTREME_MOMENTUM_THRESHOLD and
                            vol_ratio > SignalScorer.EXTREME_VOLUME_THRESHOLD and
                            price_position < SignalScorer.EXTREME_POSITION_LOW
                    )
