...
MAX_ULTRA_SHORT_PERIOD_CHANGE_FOR_LEADER = 13.0  # 主板（%）
MAX_ULTRA_SHORT_PERIOD_CHANGE_FOR_LEADER_NON_MAIN = 19.0  # 非主板（%）
...
def archive_leader_sheets(wb, sheets_to_archive, output_file):
    ...
    archive_prefix = f"{base_name}_龙头归档"

    # 归档主文件：仍放在 excel/ 下，兼容历史：{base}_龙头归档.xlsx
    main_archive_file = os.path.join(output_dir, f"{archive_prefix}.xlsx")

    # 拆分文件放在：excel/leader_archives/
    split_dir = os.path.join(output_dir, "leader_archives")
    os.makedirs(split_dir, exist_ok=True)

    def _new_split_path() -> str:
        """创建一个不冲突的拆分文件路径：{prefix}_{YYYYMMDD}.xlsx（必要时加 _dupN）"""
        split_date = datetime.now().strftime('%Y%m%d')
        candidate = os.path.join(split_dir, f"{archive_prefix}_{split_date}.xlsx")
        if not os.path.exists(candidate):
            return candidate
        dup = 2
        while True:
            candidate2 = os.path.join(split_dir, f"{archive_prefix}_{split_date}_dup{dup}.xlsx")
            if not os.path.exists(candidate2):
                return candidate2
            dup += 1

    def _list_split_files() -> List[Tuple[str, str, float]]:
        """
        返回拆分文件列表：[(path, split_date_yyyymmdd, mtime), ...]
        """
        ...
        for fn in os.listdir(split_dir):
            ...
            try:
                ...
                mtime = 0.0
            items.append((path, date_str, mtime))
        return items

    def _pick_active_archive_path() -> str:
        """
        选择当前“写入目标”。
    ...
    active_path = _pick_active_archive_path()
    if os.path.exists(active_path):
        print(f"开始归档 {len(sheets_to_archive)} 个龙头工作表到: {active_path}")
        archive_wb = load_workbook(active_path)
        print(f"加载已有归档文件: {active_path}")
    else:
        print(f"开始归档 {len(sheets_to_archive)} 个龙头工作表到: {main_archive_file}")
        archive_wb = Workbook()
        if 'Sheet' in archive_wb.sheetnames:
            archive_wb.remove(archive_wb['Sheet'])
        active_path = main_archive_file
        print(f"创建新的归档文件: {active_path}")

    def _split_archive_part(active_wb_obj, active_path_obj: str) -> Tuple[object, str]:
        """
        当 active 超过阈值时，把 active 按 sheet 数对半拆成：
        ...
        mid = len(leader_names) // 2
        left_names = set(leader_names[:mid])
        right_names = [n for n in leader_names if n not in left_names]

        right_wb = Workbook()
        if 'Sheet' in right_wb.sheetnames:
            right_wb.remove(right_wb['Sheet'])

        for sheet_name in right_names:
            source_ws = active_wb_obj[sheet_name]
            target_ws = right_wb.create_sheet(title=sheet_name)
            copy_worksheet(source_ws, target_ws)

        # 将 active_wb_obj 剩余为左半
        for sheet_name in right_names:
            if sheet_name in active_wb_obj.sheetnames:
                active_wb_obj.remove(active_wb_obj[sheet_name])

        # 保存 left 覆盖原文件
        active_wb_obj.save(active_path_obj)
        print(f"  触发归档拆分：左半保存 {len(left_names)} 张到 {active_path_obj}")

        # 保存 right 到新拆分文件
        new_path = _new_split_path()
        right_wb.save(new_path)
        print(f"  触发归档拆分：右半保存 {len(right_names)} 张到 {new_path}")
        return right_wb, new_path
    ...
    for sheet_info in sheets_to_archive:
        ...
        # 写入前：如果 active 已达到阈值，则先拆分再写入（对应“超限后再分，并继续写新文件”）
        while len(_leader_sheet_names_in_wb(archive_wb)) >= MAX_LEADER_ARCHIVE_SHEETS:
            archive_wb, active_path = _split_archive_part(archive_wb, active_path)
    ...
    if archived_count > 0:
        try:
            archive_wb.save(active_path)
            print(f"成功归档 {archived_count} 个工作表到: {active_path}")
        except Exception as e:
            print(f"保存归档文件时出错: {e}")
    else:
        print("没有新的工作表需要归档")
