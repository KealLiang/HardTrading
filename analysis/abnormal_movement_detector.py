...
class AbnormalMovementDetector:
    ...
    def check_abnormal_movement(self, stock_code, end_date):
        ...
        try:
            # 检查连续3个交易日偏离值累计±20%
            deviation_3d = self.calculate_deviation_values(stock_code, end_date, 3)
            if len(deviation_3d) >= 3:
                cumulative_deviation = sum(deviation_3d)
    ...
    def check_severe_abnormal_movement(self, stock_code, end_date):
        ...
        try:
            ...
            deviation_10d = self.calculate_deviation_values(stock_code, end_date, 10)
            if len(deviation_10d) >= 10:
                cumulative_deviation_10d = sum(deviation_10d)
            ...
            deviation_30d = self.calculate_deviation_values(stock_code, end_date, 30)
            if len(deviation_30d) >= 30:
                cumulative_deviation_30d = sum(deviation_30d)
    ...
    def get_warning_message(self, stock_code, end_date, period_data=None):
        ...
        try:
            ...
            # 检查10日偏离值预警（严重异动）
            deviation_10d = self.calculate_deviation_values(stock_code, end_date, 10)
            if len(deviation_10d) >= 5:  # 至少有5天数据才能预警
                current_cumulative_10d = sum(deviation_10d)
            ...
            # 检查30日偏离值预警（严重异动）
            deviation_30d = self.calculate_deviation_values(stock_code, end_date, 30)
            if len(deviation_30d) >= 15:  # 至少有15天数据才能预警
                current_cumulative_30d = sum(deviation_30d)
            ...
            deviation_3d = self.calculate_deviation_values(stock_code, end_date, 3)
            if len(deviation_3d) >= 2:  # 至少有2天数据才能预警
                current_cumulative = sum(deviation_3d)
