...
class LianbanAnalyzer:
        ...
        for code, date_records in tqdm(stock_records.items(), desc="筛选股票"):
            ...
            for period_records in lianban_periods:
                if self._meets_criteria(code, period_records):
                    stock_info = self._build_stock_info(code, period_records)
                    if stock_info:
        ...
        for date_col in sorted_dates:
            ...
            if prev_date_str is None:
                ...
                try:
                    ...
                    gap = len(trading_days) - 1  # 减1因为包含了起始日期
    ...
    def _build_stock_info(self, code: str, date_records: Dict) -> Optional[StockBoardInfo]:
        ...
        Args:
            code: 股票代码
            date_records: 该股票在各日期的数据（单个连板周期）
    ...
    def _find_first_board_date(self, code: str, period_start_date: str) -> Optional[str]:
        ...
        查找逻辑：从连板周期开始日期往前查找，找到最近的一个首板日期
        
        Args:
            code: 股票代码
            period_start_date: 该连板周期的开始日期（YYYYMMDD格式）
        ...
        # 返回最接近连板周期开始日期的首板日期
        found_dates.sort(reverse=True)  # 降序排列
