...
# 如果需要自定义形态模式识别，可以在这实现
def self_pattern_implement(close_arr, open_arr, pattern_results):
...
class TALibPatternStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('self_pattern', False),  # 是否使用自定义的识别模式
        ('debug', False),  # 是否输出更多调试信息
    )
    ...
    def next(self):
        ...
        try:
            ...
            # 如果需要，添加自定义形态检测
            if self.p.self_pattern:
                self_pattern_implement(close_arr, open_arr, pattern_results)
