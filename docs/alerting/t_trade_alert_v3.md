...
信号评分采用多维度综合评估，总分0-100分。详细的评分逻辑和参数配置见：
📖 **[信号评分系统配置指南](./signal_scoring_config.md)**
...
| 维度 | 权重 | 说明 |
|------|------|------|
| **技术指标** | 0-20分 | RSI超买/超卖程度 |
| **价格位置** | 0-30分 | 相对60根K线高低点的位置 |
| **布林带偏离** | 0-15分 | 偏离布林带上下轨的程度 |
| **量能形态** | 0-35分 | 量价关系+拉升/下跌期判断 |
| **做T适用性** | -55~+10 | 趋势+波动率+横盘+极端行情 |

### 信号分级

| 分数区间 | 强度标识 | 含义 | 建议操作 |
|---------|---------|------|----------|
| **85-100** | ⭐⭐⭐强 | 多维度指标极值+量能确认 | 重仓操作，高概率机会 |
| **65-84** | ⭐⭐中 | 标准信号，技术指标达标 | 正常操作，常规机会 |
| **55-64** | ⭐弱 | 边缘信号，部分指标弱化 | 谨慎操作或观望 |
| **<55** | 🚫过滤 | 低质量信号，不触发 | 不操作 |

### 极端行情降级（v1.5新增）⭐

**对称设计**，防止拉升中途误卖和杀跌中途误买：

| 场景 | 触发条件 | 处理 |
|------|---------|------|
| **极端拉升** | 10根涨>3% + 量>3x + 位置>95% | 总分×30% |
| **极端杀跌** | 10根跌>3% + 量>3x + 位置<5% | 总分×30% |

**可调参数**：详见[配置指南](./signal_scoring_config.md)
...
1. **跨日数据处理**: 
   - **量价确认和背离检测**: 会自动过滤跨日数据，仅使用当日K线
   - **混合/右侧模式跨日回退**: 开盘前2-3分钟（如09:31-09:32）因前2根K线是前一日的，会自动回退到左侧模式（避免信号缺失）
   - **实战案例**: 10-24早盘09:31出现RSI 82.4的完美卖点，若不回退会被过滤
...
### 🐛 坑点1: 开盘初期信号缺失（已修复 v3.2）

**问题描述**:
- 使用 HYBRID 或 RIGHT 模式时，开盘前2-3分钟（如09:31-09:32）的信号会被完全过滤
- 即使出现 RSI>80 的完美卖点也无法触发

**原因分析**:
- HYBRID/RIGHT 模式需要检查前2根K线的RSI趋势（`rsi_prev`、`rsi_prev2`）
- 开盘初期前2根K线是前一日的（跨日），原逻辑直接返回 `None` 跳过此K线
- 导致高质量信号（如600078 10-24早盘09:31 RSI 82.4）被错误过滤

**修复方案**:
```python
# 修复前（v3.0-v3.1）
if date_current != date_prev or date_current != date_prev2:
    return None, "当日数据不足（跨日）", 0  # ❌ 直接跳过

# 修复后（v3.2）
if is_cross_day:
    mode = 'LEFT'  # ✅ 回退到左侧模式（不需要历史RSI）
```

**影响范围**:
- 仅影响 HYBRID/RIGHT 模式
- 每日开盘前2-3分钟（约09:31-09:33）
- LEFT 模式不受影响

**实战验证**:
- 修复后能捕获 600078 在 10-24 09:31 的 RSI 82.4 卖点（价格8.78）
- 该信号质量极高（⭐⭐⭐强信号），修复前被完全遗漏

**相关代码**:
- `alerting/t_trade_alert_v3.py` 第490-508行
- 修复日期：2025-10-24
