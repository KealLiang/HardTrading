...
class SynonymManager:
    ...
    def generate_updated_groups(self, reasons_data, current_groups=None):
        ...
        print(
            f"其中保留了 {len(original_group_order)} 个原有分组，新增了 {len(updated_groups) - len(original_group_order)} 个分组")
    ...
    def update_from_latest_file(self, debug_phrases=None):
        """
        从最新的涨停原因文件更新同义词分组
        
        Args:
            debug_phrases: 需要调试的词组列表，将打印这些词组与各个组的匹配度
        ...
        # 如果有需要调试的词组，先打印它们与现有分组的匹配度
        if debug_phrases and len(debug_phrases) > 0:
            self.debug_syn_print(debug_phrases)
    ...
    def debug_syn_print(self, debug_phrases):
        print("\n=== 调试模式：打印指定词组与各组的匹配度 ===")
        # 加载当前的synonym_groups
        current_groups = self.load_current_synonym_groups()
        for phrase in debug_phrases:
            ...
            for group_name, synonyms in current_groups.items():
                # 计算与组名的相似度
                similarity_with_name = self.classifier.calculate_similarity(phrase, group_name)
                ...
                for syn in synonyms:
                    if '%' in syn:
                        ...
                        if regex.search(phrase):
                            matched_by_wildcard = True
                            group_similarities.append((group_name, 1.0, "通配符匹配"))
                            break
                ...
                # 批量计算与组内所有词的相似度
                similarities = self.classifier.calculate_similarities_batch(phrase, synonyms)
                max_similarity = max([similarity_with_name] + similarities)

                # 记录相似度
                group_similarities.append((group_name, max_similarity, "语义相似"))

            # 按相似度排序并显示前x个
            group_similarities.sort(key=lambda x: x[1], reverse=True)

            print(f"{'组名':<20} {'相似度':<10} {'匹配类型':<10}")
            print("-" * 40)
            for group_name, similarity, match_type in group_similarities[:PRINT_TOP_N]:
                print(f"{group_name:<20} {similarity:.4f}    {match_type}")

            # 判断是否会被分类到某个组
            if group_similarities and group_similarities[0][1] >= self.threshold:
                best_group = group_similarities[0][0]
                print(f"\n结论: 【{phrase}】将被分类到【{best_group}】组 (相似度: {group_similarities[0][1]:.4f})")
            else:
                print(
                    f"\n结论: 【{phrase}】不会被分类到任何现有组 (最高相似度: {group_similarities[0][1]:.4f} < 阈值 {self.threshold})")
        print("\n=== 调试结束 ===\n")
...
def main():
    ...
    parser.add_argument("--debug", "-d", nargs="+", help="需要调试的词组列表，例如：-d '机器视觉' 'AI芯片'")
    ...
    # 如果指定了调试词组，则进入调试模式
    if args.debug:
        print(f"调试模式：将分析以下词组的分组匹配度: {args.debug}")
        manager.update_from_latest_file(debug_phrases=args.debug)
        return
