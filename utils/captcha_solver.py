...
class SimpleCaptchaSolver:
    ...
    def _close_eastmoney_popup(self):
        ...
        # 尝试通过样式定位关闭按钮
        close_elements = self.driver.find_elements(By.CSS_SELECTOR,
                                                   "img[style*='position: absolute'][style*='cursor: pointer']")
    ...
    def _solve_iframe_captcha(self):
        ...
        for iframe in iframes:
            ...
            if any(keyword in iframe_src.lower() for keyword in ["captcha", "verify"]):
                ...
                success = False
                try:
                    success = self._solve_captcha_in_current_frame()
                finally:
                    # 切换回主框架
    ...
    def _solve_captcha_in_current_frame(self):
        ...
        try:
            ...
            logging.info("未找到主要滑块元素，尝试查找刷新按钮")
            # 尝试查找刷新按钮并点击
            try:
        ...
        try:
            ...
            logging.info(f"ddddocr检测失败, {e}")
            distance = img.width * 0.3

        # 执行滑动
        return self._perform_slide_with_selectors(distance)

    def _perform_slide_with_selectors(self, distance):
        """
        查找滑块并执行滑动
        """
        # 查找滑块元素
    ...
    def _solve_generic_captcha(self):
        ...
        clickable_elements = self.driver.find_elements(By.CSS_SELECTOR,
                                                       "button, input[type='button'], [onclick], [role='button']")
