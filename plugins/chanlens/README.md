# ChanLens 缠论透镜

自绘 K 线 + 缠论结构叠层的浏览器扩展。**所有源码都在你手里**，觉得哪条规则不对，直接改。

## 为什么是"自绘"而不是"叠加在原图上"

StructGuide 那类插件把结构画在网站自己的 canvas 上，坐标要靠"数格子"反推，
网站一改版、DPR 一变、缩放一动就会错位。本插件反过来：**K 线也是我们自己画的**，
每个像素坐标都是自己算出来的，物理上不可能错位。

代价：和原网站自带指标不能共存。收益：坐标永远准、想加什么指标自己加、
多级别联动（叠加方案根本做不到）成为可能。

## 形态：不是站点叠加，而是独立 App

因为 K 线是自绘的，一旦不需要"盖在原图上"，"必须在某个网站里打开"就失去了意义。
所以本插件的主形态是**扩展自己的全屏页面**（`chrome-extension://…/app/app.html`）：
不依赖任何网站，不需要起本地服务器，同时因为扩展页面的 origin 为 null，
拿数据依旧不受 CORS 限制。

站点注入脚本只保留一个用途：你在东财/雪球看盘时，右下角点一下「缠」，
把当前这只股票带进 App。

数据层在 `core/market.js`（纯 fetch、无 chrome 依赖），App 页面**直连**它
（扩展页面有 `host_permissions`，天然免 CORS）；background 只做缓存与
content script 的代理兜底，App 不再依赖消息转发，不会因 service worker
的休眠/重启窗口卡住界面。

## 安装（Chrome / Edge）

1. 打开 `chrome://extensions`（Edge 是 `edge://extensions`）
2. 右上角打开「开发者模式」
3. 点「加载已解压的扩展程序」，选择本目录 `D:\Trading\plugins\chanlens`
4. **点击浏览器工具栏的 ChanLens 图标**打开 App（建议把它固定到工具栏）
   - 若当前正好在个股页面（东财/雪球/新浪/同花顺），会自动带上该只股票
   - 在个股页面也可以点右下角浮动的「缠」按钮进入

App 打开后：顶栏输入代码或拼音/汉字（如 `600519` / `茅台`）搜索，
回车或点选即加入自选；左侧自选股列表点击秒切，**Alt + ↑/↓** 在自选股间循环，
列表项上右键删除。也可直接用 `chrome-extension://<扩展ID>/app/app.html?code=600519` 直达。

**批量导入自选**：顶栏「批量导入」按钮，粘贴一整段即可：

```
600519
SH600036
000001, 300750
601318 中国平安
```

支持一行一个、逗号/空格/分号分隔、`sh/sz` 前缀与点号写法，自动**去重**；
`代码 名称` 混写会带上名称，其余缺失名称在后台并发自动补全。
另外在搜索框直接粘贴含多个代码的文本，会**自动转成批量导入**，`Ctrl+Enter` 确认。

**批量删除自选**：自选栏右上「批量删除」打开弹窗，默认全选，可逐项取消或一键全选/全不选，
点「删除 (Ctrl+Enter)」二次确认后执行，`Esc`/点遮罩关闭。
列表项的名称在获取后会写回本地存储，重启浏览器不丢失；旧数据里缺失/退化为代码的名称，
打开 App 时会自动补全。

**信号扫描（周期可选，默认日线）**：自选栏右上「扫信号」按钮；扫描周期在**设置面板最底部的
「自选扫描」分组**里选（15分/30分/60分/日线/周线，默认日线），对全部自选做一次所选周期「正在形成」的
买卖点扫描，结果以小徽标（`一买`/`三卖`…）显示在每只自选项的涨跌幅上方，买的红底、卖的绿底，
**虚线边框（文字本色红/绿）= 尚未确认**（分型右侧可能修订），悬停可看周期/成立时间/力度比/
滞后根数。切换周期自动显示该周期上次缓存（零请求），点「扫信号」才联网。
**各周期缓存独立分桶**（`周期|代码`），互不覆盖；旧版单层缓存自动迁移。

减负设计（数据压力最小化）：

- 每只只取 **200 根** K（≈10 个月日线，实测足够跑出稳定结构，比看图的 800 根省 4 倍）
- **一次点击**并发 3 逐个回填；30 分钟内扫过的自动跳过（`Shift+点击` 强制全部重扫）
- 结果持久化在 `chanlens.scan.v1`，**重开 App 零请求**直接显示上次结果；纯手动触发，无定时器
- 「正在形成」判据：信号成立位 `readyK` 距最新 K ≤ 10 根，取最新的一个；不引入未来函数
- 引擎正确性与长度稳定性验证见 `tools/scan_probe.js`（100/150/200/256 根检出同一信号）

**自选涨跌幅**：每个自选项右侧（与代码同行、右对齐）实时显示最近一根日K的涨跌幅，
红涨绿跌，停牌显示 `—`。打开 App 拉取一次，另有 10 分钟一次的兜底刷新（后台标签页自动暂停，省流量），
日常看图**直接刷新页面**即可拿到最新值。
数据走东财批量接口（一次请求拿全部自选），东财不可达时自动降级腾讯批量行情，再不行逐只兜底。
侧栏点击标的即可切换，**为手机版做了空间预留**（右侧涨跌幅不占新行）。

**自选分类（文件夹）**：自选栏右上「+ 分类」新建分类（如 核心池 / 观察池），标的按分组折叠展示：

- 右键标的 → **移动到某分类** / 新建分类并移入 / 从自选中删除；也可以直接**拖拽**标的到分组头上
- 右键分组头 → 重命名 / 删除分类（组内标的自动退回「未分类」，不删除标的）/ 折叠展开 / 设为默认落点
- 分组头左侧高亮的分类是**默认落点**：「+ 自选」「批量导入」的新标的默认进这个分类（批量导入弹窗里也可现场换）
- 折叠状态、分组顺序、默认落点都会持久化（`chanlens.watchcats.v1`），重启不丢
- 旧自选数据没有分类字段，自动归入「未分类」，完全向后兼容；`Alt + ↑/↓` 按「分组顺序 → 组内加入顺序」循环

## 功能

| 功能 | 说明 |
|------|------|
| 缠论结构 | 包含处理 → 分型 → 笔 → 线段 → 中枢，实线=已确认，虚线=待确认 |
| K 线形态切换 | 设置面板「显示 → K 线形态」：普通 K 线 / **缠论 K 线**（含包处理后的合并序列，只画高低点区间的简洁矩形柱——无开收盘/影线/红绿，量柱中性色，十字光标显示区间与合并根数）。纯显示层变换，`chart/mergedview.js` 负责把全部结构下标重映射到缠论 K 线，判定结果与无未来函数性质不变；MACD 按区间末根采样、量额取区间之和 |
| 背驰 | 相邻两笔同向走势力度对比（MACD 柱面积），标注比值 |
| 三类买卖点 | 一类(背驰极值) / 二类(回抽不破) / 三类(回抽不入中枢) |
| 多级别联动 | 最多三个周期同屏，任意一张缩放/平移，其余**按同比同步**（各周期数据跨度差几十倍，同步绝对时间窗口会跳变，故同步缩放倍数与相对位移）；顶栏「**全览**」按钮一键显示完整区间 |
| 缠论心法 | 点左上角「缠」logo 弹出**实操速查卡**（开仓三句话 / 级别纪律 / 出场四层 / 特殊情况 / 四条禁令），内容与 `docs/缠论实操速查.md` 同步维护 |
| 全参数可调 | 笔间隔、线段算法、中枢口径、背驰阈值、MACD 参数……全在右侧面板 |
| 导出 | 一键导出结构 JSON（含笔/线段/中枢/背驰/买卖点），喂给本地 Python 回测 |
| 截图 | 保存当前主图 PNG |

## 目录结构

```
chanlens/
├── manifest.json            MV3 清单
├── background/
│   └── service_worker.js    行情代理（绕开页面 CORS），东财主源 + 新浪备用
├── content/
│   ├── adapters.js          站点适配：从 URL/DOM 识别股票代码
│   ├── datasource.js        行情请求 + 时间戳标准化 + 导出（App 与 content 共用）
│   ├── launcher.js          站点内的浮动入口：识别代码 → 跳 App
│   └── content_legacy.js    旧模式（页面内直接画图）入口，已不从 manifest 引用
├── app/
│   ├── app.html             ★ 主界面：扩展自己的全屏 App 页
│   ├── app.css              外壳布局（顶栏/搜索/自选侧栏）
│   └── app.js               搜索、自选股批量导入与快速切换、串起面板与渲染
├── core/
│   ├── indicators.js        EMA / MACD / 区间力度
│   ├── chan.js              ★ 缠论引擎（纯函数、无 DOM，Node 可直接测）
│   └── market.js            数据层：东财 K线(主)+新浪(备)、搜索、名称反查（SW 与 App 共用）
├── chart/
│   └── renderer.js          K 线 + 叠层 canvas 渲染、缩放/平移/十字光标、zoomBy/panBy/zoomFull
├── ui/
│   ├── panel.js             控制面板（参数表驱动，加参数=加一行 schema）
│   ├── panel.css            面板样式，含嵌入模式（App 外壳使用）
│   └── launcher.css         站点浮动按钮样式
├── python/
│   ├── chan_engine.py       ★ 同一套算法的 Python 版（给 D:\Trading 回测用）
│   └── test_chan_engine.py  与 JS 引擎交叉验证
├── tests/
│   └── test_chan.js         引擎单元测试（node 直接跑）
├── tools/
│   ├── fetch_testdata.py    拉真实行情生成离线 fixture
│   ├── debug_dump.js        打印每一层中间结果（改参数后排查为什么）
│   ├── compare.js           对比不同参数组合的产出 + 背驰诊断
│   └── make_icons.py        生成图标
└── icons/
```

## 常用命令

```bash
# 跑引擎测试（41 项）
node tests/test_chan.js

# 用真实数据打印每一层结构
node tools/debug_dump.js tools/testdata/600519_daily.json
# 带参数
node tools/debug_dump.js tools/testdata/600519_daily.json minFxGap=2 segAlgo=\"feature\"

# 拉新数据做 fixture（股票:周期）
python tools/fetch_testdata.py 600519:daily 000001:30m

# 未来函数 / 重绘检测（改引擎后必跑）
node tools/walkforward.js                  # JS 引擎：滚动推进比对
python tools/walkforward.py                # Python 引擎：同上
NODE_EXE=/path/to/node python tools/walkforward.py   # 附带 JS/PY 交叉核对

# Python 引擎测试（含与 JS 的交叉验证）
python python/test_chan_engine.py
```

## 无未来函数的保证（做回测前必读）

**已验证的结论**：算法层不存在未来函数。用 4 份真实数据 × 4 组参数，
逐段推进到 140 个切点，比对「截止 T」与「全量」两种算法的结果：

| 层级 | 结论 | 证据 |
|------|------|------|
| MACD | 因果：截断序列与全序列同位数值完全相同 | 递归 EMA，`hist[i]` 只依赖 `closes[0..i]` |
| 包含处理 / 分型 | 因果 | 左→右单向扫描 |
| 笔 / 线段 / 中枢 | 除最后一个外，历史结构永不改写 | 滚动比对零差异 |
| 背驰 / 买卖点 | 除最后一组外，历史信号永不改写 | 滚动比对零差异 |

**正在形成的最后一个结构一定会变**（缠论本身的性质，图上画成虚线）。
回测时必须丢掉每层最后一个元素：

```python
r = chan_engine.analyze_klines(klines[:i], {})     # 只用截止 i 的数据
settled_bis  = r['bis'][:-1]        # 最后一笔待确认，不可用于产生信号
settled_segs = r['segs'][:-1]
settled_zs   = r['zhongshus'][:-1]
settled_divs = r['divergences'][:-1]
```

## 买卖点的构成：只有「一类」直接落在背驰上

| 级别 | 判定依据 | 是否需要背驰 |
|------|----------|--------------|
| 一类买/卖 | 本级别第 3 段（线段或笔）创新极值但力度/幅度衰减 → 背驰段的极值点 | **是**，直接由 `divergences` 生成 |
| 二类买/卖 | 一类之后，跳过反弹段，下一次同向回抽未破一类的极值 | 间接依赖（前提是一类存在） |
| 三类买/卖 | 离开中枢后的第一次反向回抽，低点不落回中枢区间（`pp.low > ZG` / `pp.high < ZD`） | 否，纯中枢结构判定 |

三者都符合缠论教材的定义，但**不要把三买/三卖误读成"背驰信号"** —— 它们的产生与背驰无关。

## 可得性：`_k` 是画图位置，不是信号发出时刻

缠论要求分型右侧确认，一类买卖点还必须等到"第 3 段走完"才成立。这两层滞后是算法性质：

| 字段 | 含义 | 回测用途 |
|------|------|----------|
| `p['_k']` | 画图所在的 K 线（背驰段的极值点 / 回抽段的末端） | **只用于画图**，不可用于成交 |
| `p['readyK']` | 该信号**成立**所需信息的最后一根 K 线 | 回测入场：`entry = readyK + 1` |
| `p['confirmed']` | `lastK - readyK >= 3`，右侧确认走够了 | 图上未确认的信号画成半透明虚线圈 |

实测（4 份真实数据 × 4 组参数 × 153 个买卖点，`tools/point_audit.js`）：

- 相对 `readyK`：信号恒在 `readyK` 之后再走 1 根才可见（153/153 样本，中位=最大=1）
  → **`readyK + 1` 开盘成交是严格可实现的保守口径**，不存在任何"信号早于所需信息"的样例；
- 相对标记位置 `_k`：一类买/卖点中位数滞后 1 根，但极端可达 10 根（30 分钟级，背驰极值点在线段内部时）。
  **若回测直接拿 `klines[_k].close` 成交，那就是入场前视偏差** —— 这个坑比"重绘"更常见。

```python
for p in r['points']:
    if p['readyK'] + 1 >= len(klines):   # 尚未成型，跳过
        continue
    entry_bar  = p['readyK'] + 1
    entry_px   = klines[entry_bar]['o']  # 保守：下一根开盘
    # 而不是 klines[p['_k']]['c']
```

例外出现在数据最末端的**未定型背驰段**：同一个 markers 位置的价格会被后续更极值改写
（茅台 30 分钟 - 169 处底背驰在推进过程中 price 由 1288 修订为 1286.1）。
这是"最后一组结构必然修订"的一部分，不影响 `readyK + 1` 的成交口径，
但再次说明：**末端信号不可用于回测**。

**算法之外仍有三个真实重绘源，请自行规避**：

1. **复权方式**（最容易踩）：本插件默认 `fqt=1` 前复权，历史价格会在每次
   除权除息后被重新缩放。**回测请改用后复权 hfq** 并锁定数据快照；前复权只适合看盘。
2. **未收盘 K 线**：盘中最后一根 bar 还在变动，基于它产生的信号会闪烁。
   实盘信号请只在 bar 收盘后取，或直接丢弃涉及最后一根 bar 的结构。
   （已验证：最后一根 bar 的盘中波动不会污染更早的历史结构。）
3. **数据源事后修正**：行情商偶尔修正历史 K 线。建议把回测数据本地落盘做版本快照。

## 二次开发指南

**改判定规则** → 全部在 `core/chan.js`，每个函数都有注释说明它实现的是缠论哪一条。
改完先跑 `node tests/test_chan.js`，确认没破坏不变量（笔首尾相接、中枢 ZD<ZG、背驰必然力度衰减）。

**加一个参数** → 在 `ui/panel.js` 的 `PARAM_SCHEMA` 里加一行，UI 和持久化自动生效；
引擎侧在 `core/chan.js` 的 `DEFAULTS` 里加同名字段即可。

**加一个网站** → 在 `content/adapters.js` 的 `SITES` 和 `fromUrl()` 里加一条正则。
因为我们是自绘，适配工作只有"识别股票代码"这一件事。

**接入回测** → 页面点「导出JSON」，或在 Python 里直接：
```python
import sys; sys.path.insert(0, r'D:\Trading\plugins\chanlens\python')
from chan_engine import analyze_df
r = analyze_df(df)          # df 来自 fetch.astock_data
print(r['stats'])
```

## 已知边界（诚实声明）

- **线段算法有两种**：默认 `simple`（回调破前低即终结，切分均衡），
  可切换 `feature`（特征序列法，教科书标准，但需要 6 笔以上才能确认第一段，所以线段会很长、末段经常"待确认"——这不是 bug，是缠论本身的性质）。
- **中枢默认用笔算**（`zsSource:'bi'`），颗粒度接近主流工具；教科书口径请切 `seg`。
- **背驰默认滚动三笔比较**（稳定、一定有信号）；`divMode:'zs'` 是严格的
  "被同一中枢隔开的连接波"模式，但中枢首尾相接时它一个信号都不会给。
- 中枢扩展/扩张（中枢升级）尚未实现，只做了延伸。
- 数据来自东方财富/新浪公开接口，仅用于研究与复盘，**不构成任何投资建议**。

## 与 StructGuide 的对比

| | StructGuide | ChanLens |
|---|---|---|
| 源码 | 闭源，无法调整 | 全部源码在本目录 |
| 画在哪 | 叠在网站 canvas 上（可能错位） | 自绘（坐标不可能错） |
| 背驰/买卖点 | 无 | 有，阈值可调 |
| 多级别联动 | 无 | 三周期同屏同比联动 + 「全览」按钮 |
| 参数 | 只有图层开关 | 全部算法参数可视化 |
| 回测接入 | 无 | JSON 导出 + 同算法 Python 引擎 |
